    ...
    class Map {
        ...
        for (i = 0, len = desc.length; i < len; i++) {
          ...
          bounds.extend(
            new google.maps.LatLng(
              value.latlon.split(",")[0],
              value.latlon.split(",")[1]
            )
          );
        }
      ...
      selectLandingPoint(name, latLng) {
        ...
        this.infoBox.setContent(
          `<div class="infoBoxContent"><div class="infoBoxPointer"></div>${name}</div>`
        );
      ...
      resetBounds() {
        this.gmap.setZoom(2);
        return this.gmap.setCenter(new google.maps.LatLng(-6.2, 106.816666));
      }
      ...
      setEvents() {
        ...
        this.cables.addListener("click", (event) => {
          return jQuery(location).attr(
            "href",
            `#/submarine-cable/${event.feature.getProperty("slug")}`
          );
        });
        return this.landings.addListener("click", (event) => {
          return jQuery(location).attr(
            "href",
            `#/landing-point/${event.feature.getProperty("slug")}`
          );
        });
      }
